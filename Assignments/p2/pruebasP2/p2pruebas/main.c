...
int main(int argc, char *argv[]) {
    ...
    // ComprobaciÛn de que el n˙mero de argumentos de la lÌnea de orden es correcto
    if (argc < 2)
...
int estado(char *fichero) {
    ...
    // Separa los n˙mero 'major' y 'minor' que identifican al dispositivo
    //	en el que est· alojado este inodo; es decir, el volumen
    ...
    // Muestra el n˙mero de inodo; junto con el valor anterior
    ...
    //	y alg˙n otro modo de operaciÛn adicional
    //	(S_ISxxx son m·scaras de bits)
    printf(" Tipo: ");
    ...
    // Muestra el n˙mero de enlaces rÌgidos de este inodo
    printf("\n");
    printf(" Enlaces: %ld\n", (long int)buf.st_nlink);
    
    // Muestra las identificaciones de usuario y de grupo,
    // n˙merica y simbÛlicamente
    //	La funciÛn getpwuid() consulta el fichero /etc/passwd
    //	y la funciÛn getgrgid() consulta el fichero /etc/group
    ...
    // Si el fichero es de tipo especial muestra los n˙emros
    ...
    // Muestra el tamaÒo del fichero
    printf(" Tamaño: %ld bytes.\n", buf.st_size);
    
    // Muestra las fechas asociadas al fichero en formato cadena de caracteres.
    //	stat() poroporciona los tiempos como n˙meros de tipo time_t
    //	La funciÛn localtime() transforma un tiempo de tipo time_t
    //	 en una struct tm
    //	La funciÛn asctime() transforma un tipo de tipo struct tm
