 ...
 ** y el hilo se quedará bloqueado hasta que el hilo consumidor invoque a mbox_fetch().
...
void mbox_post( struct sys_mbox *mbox, void *msg)
{
	...
	//Comprobamos que no haya espacio
	...
	//En caso de que este vacio antes
	if(is_empty_cbuffer_t(mbox->cbuffer) != 0)
	{
		//Informamos
		sys_sem_signal(mbox->not_empty);
	}
	//Insertamos el mensaje en el buffer
 ...
 ** El hilo invocador comprobará si hay elementos en el buffer circular. Si no hay ninguno, se quedará bloqueado hasta que haya elementos.
...
void* mbox_fetch(struct sys_mbox *mbox)
{
	...
	//Bloqueamos el hilo mientras este vacio el buffer
	...
	//Comprobamos que hay hilos esperando
	while((mbox->wait_send) > 0)
	{	
		//Le despertamos
	...
	pthread_mutex_unlock(mbox-> mutex);
	//Devolvemos el mensaje extraido
	return msg;
}
/*-----------------------------------------------------------------------------------*/
