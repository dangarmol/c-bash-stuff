...
int execute_random_service(message_t* msg) {
	//Inicializamos el puntero al content
        int *numeroAleatorio = msg->content;
	
	//Incluimos el numero aleatorio en el contenido de content
	*numeroAleatorio = rand();

	//Lo guardamos en logfile
	fprintf(logfile," SERVER random: El numero generado es el: %d\n",(int)numeroAleatorio);
	return 0;
}
