...
sched_class_t* active_sched_class=&rr_dq_sched; /* Default RR */
...
static void init_runqueue(runqueue_t* rq, int cpu) {
	...
	init_slist(&(rq->int_tasks),sizeof(task_t),offsetof(task_t,rq_links));
	init_slist(&(rq->cpu_tasks),sizeof(task_t),offsetof(task_t,rq_links));
